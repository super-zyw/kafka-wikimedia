...
import json
import sys
from utils.evenetHandler import EventHandler
    ...
    for event in EventSource(url_string, headers=custom_headers):
        if event.event == 'message':
            try:
                ...
                eventHandler.on_message(
                    key = str(wiki['id']),
                    #msg = avro_serializer(wiki, SerializationContext(topic, MessageField.VALUE))
                    msg = json_serializer(wiki, SerializationContext(topic, MessageField.VALUE))
